...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a4635b395fb64aaab8417f63e64291d4~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp) CNAME（意为：别名）：在域名解析中，实际上解析出来的指定域名对应的IP地址，或者该域名的一个CNAME，然后再根据这个CNAME来查找对应的IP地址。
...
**图示：** ![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/efbcb31418be45eaa848d107b107c4eb~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp) **代码实现：**
...
1. 1. 1. 1. 静态资源加hash后缀，根据文件内容计算hash
         2. 文件内容不变，则hash不变，则url不变
         3. url和文件不变，则会自动触发http缓存机制，返回304

1. 1. 1. **SSR**

1. 1. 1. 1. 服务端渲染：将网页和数据一起加载，一起渲染
...
1. 1. CSS放在head，JS放在body下面
   2. 尽早开始执行JS，用DOMContentLoaded触发
   3. 懒加载(图片懒加载，上滑加载更多)

![img](https://raw.githubusercontent.com/liyunfuyyyy/img-url/master/1646702939010-12b8b361-0f28-41c5-a595-7465ef43664a.png)

1. 1. 对DOM查询进行缓存

![img](https://raw.githubusercontent.com/liyunfuyyyy/img-url/master/1646702975812-876ab199-d99f-43e7-b688-3c666efb9f1d.png)

1. 1. 频繁DOM操作，合并到一起插入DOM结构
   2. 节流throttle 防抖 debounce

1. 1. 1. **防抖**

1. 1. 1. 1. 监听一个输入框，文字变化后触发change事件
         2. 防抖：用户输入结束或暂停时，才会触发

1. 1. 1. **节流**

1. 1. 1. 1. 无论多快，都隔一定时间触发一次
